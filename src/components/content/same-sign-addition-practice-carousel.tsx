...
const PRACTICE_ITEMS: PracticeItem[] = [
  {
    ...
    label: '1',
    question: '(+4)+(+3)을 계산하면?',
    hint: '공통의 부호 +를 쓰고, 절댓값 4와 3을 더해요.',
    solvedNote: '공통의 부호 +, 절댓값의 합 4+3=7이므로 +7이에요.',
  },
  {
    id: 'negativeSum',
    label: '2',
    question: '(−5)+(−2)를 계산하면?',
    hint: '공통의 부호 −를 쓰고, 절댓값 5와 2를 더해요.',
    solvedNote: '공통의 부호 −, 절댓값의 합 5+2=7이므로 −7이에요.',
  },
  {
    id: 'sameSignRule',
    label: '3',
    question: '같은 부호의 두 수를 더할 때 먼저 정할 것은?',
    hint: '두 수가 함께 가지고 있는 부호를 생각해요.',
    solvedNote: '같은 부호의 덧셈은 공통의 부호를 먼저 쓰고 절댓값을 더해요.',
  },
]

export function SameSignAdditionPracticeCarousel({
  blank,
  solvedAnswers,
  ...
  return (
    <div className="text-left">
      ...
      <motion.div
        ...
        className="rounded-[18px] border border-[#D6DDE5] bg-[#FFFDF5] px-4 py-5 shadow-[0_10px_24px_rgba(80,60,20,0.08)] sm:px-5"
      >
        <div className="mb-4 flex items-center justify-between gap-3">
          <div className="grid size-7 place-items-center rounded-full bg-[#8B5E16] text-[14px] font-black text-white">
            {activeItem.label}
          </div>
          <div className="text-[13px] font-bold text-slate-500">같은 부호의 덧셈</div>
        </div>

        <div className="rounded-[14px] bg-white px-4 py-5 text-center shadow-sm">
        ...
        {(isShowingCorrectFeedback || isActiveSolved) && (
          <motion.div
            ...
            className="mt-4 rounded-[14px] border border-[#2E7D57]/20 bg-[#2E7D57]/10 px-4 py-2 text-center text-[15px] font-extrabold leading-[1.6] text-[#2E7D57]"
          >
